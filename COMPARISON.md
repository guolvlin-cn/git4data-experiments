...
- **缺口（要补的）**：无 SDK/编程模型/确定性重放、无持久 timers/signals/queues/virtual objects、无 worker 调度/版本化/限流/visibility UI。要么自建薄库（DBOS 式，如本实验），要么在其上加一层引擎。
