...
> 部分弥补（代价是周期性而非实时增量）。本项目的 **v3.0.11 实例尚不支持 `CREATE TASK`**（实测语法错）；
> p95/p99 百分位、`uniq`/HLL、`date_trunc` 等函数缺口与 `CREATE TASK` 无关，仍然存在。
...
| 持久 timers（睡几天/月）| ✅ | ✅ | ✅ | 无 per-instance 工作流 timer，但**较新版有内置 `CREATE TASK` cron 定时任务**（cron+RETRY+TIMEOUT）可在库内调度，用来轮询 `timers` 表触发到期项——无需外部调度器（v3.0.11 实例实测尚不支持 CREATE TASK）|
...
- **更正（内置调度）**：MatrixOne **较新版本（v26.x 文档）有内置 `CREATE TASK` 定时任务**——cron（6 段含秒）+ `WHEN` 门禁 + `RETRY` + `TIMEOUT`，单实例串行，`SHOW TASK RUNS` 看历史。这意味着「自建 durable timer / 轮询 durable-queue 表」**不需要外部调度器**，用库内 `CREATE TASK` 每秒/每分轮询触发到期项即可。（注意：本项目测试的 **v3.0.11 实例尚不支持** `CREATE TASK`，实测报语法错；该能力在更新版本提供。）
